...
Each field in \texttt{WalletSignal} maps onto a row of the
feature-inputs table introduced later
(Table~\ref{tab:scoring-features}, Section~\ref{sec:vri:features}).
The \texttt{value\_bps} field carries the wallet-class anomaly score
whose feature inputs are enumerated in the first row of that table;
\texttt{confidence\_q7} carries the calibration-channel score of the
fourth row. The \texttt{type} field selects which row the consumer
dereferences: a wallet-class signal points the consumer at the
wallet row of Table~\ref{tab:scoring-features}, while protocol- and
market-class signals point at the protocol and market rows under
their equivalent IDL structures. The \texttt{prov\_root} field is
therefore the audit hook by which a verifying consumer reaches the
underlying feature evidence set without re-deriving the score.
...
the validator loss the score contributes to. Three design choices
warrant explicit naming. First, each row binds to a concrete score
that lands in the wire envelope of Section~\ref{sec:arch}
(\texttt{value\_bps}, \texttt{confidence\_q7}) or to a validator-loss
component, not to abstract risk concepts; this lets a reviewer audit
the methodology by reading the IDL and the table together. Second,
off-chain and grey-literature feeds are admitted only in the
market-level signal class, where narrative-shock fusion divergence is
empirically informative~\cite{cong2022crypto}; wallet- and
protocol-level scores are restricted to on-chain inputs to keep the
load-bearing scoring path Sybil-resistant. Third, the validator-loss
components are flagged \textsc{deferred} consistently with the
layer-2 honesty split of Section~\ref{sec:vri}: the Brier,
Inconsistency, and Calibration channels are specified
architecturally and not yet measured against a live multi-miner
metagraph. Downstream, the \texttt{value\_bps} score feeds anomaly
haircuts and routing decisions for autonomous agents, while
\texttt{confidence\_q7} gates the agent's constitutional action
surface: an agent declares a per-signal-type minimum confidence,
below which a propagated signal is silently downweighted rather than
acted upon~\cite{bai2022constitutional}.
