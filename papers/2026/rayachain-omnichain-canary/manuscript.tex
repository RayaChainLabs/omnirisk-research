...
%% Author 2 confirmed by CEO 2026-05-07: Ahod Alghuried, Prince Sattam Bin
%% Abdulaziz University, KSA. Email / ORCID / IEEE membership grade still
%% pending --- email and ORCID placeholders below remain TBD.
...
\thanks{A. Alghuried is with the College of Computer Engineering and
Sciences, Prince Sattam bin Abdulaziz University, Al-Kharj 16278,
Kingdom of Saudi Arabia (e-mail: a.alghareed@psau.edu.sa).}%
...
%% IEEEpubid removed per CEO directive 2026-05-07; placeholder line not needed pre-acceptance.
...
layers into a single primitive. We formalize the canary-layer
...
decentralized validation subnet, and report orchestration evidence
...
reputation, behavioral risk, protocol exposure, and route health do
...
decentralized validation subnet for verifiable risk
...
scoring, and decentralized validation networks.
...
that operationalizes them. Section~\ref{sec:disc} states benefits,
...
Blockchain technology has emerged as a decentralized paradigm for
secure, peer-to-peer value exchange without reliance on centralized
...
00330023}. Cryptocurrencies and decentralized applications (DApps)
...
producing inefficiencies in asset utilization and system-wide
coordination.
...
multi-chain DeFi~\cite{ZhangZZL24}. They operate at the asset layer
and do not propagate the contextual or behavioral information attached
to those assets. Existing validation mechanisms verify the correctness
of transactions but do not assess the \emph{intent} or behavioral
patterns of participating entities~\cite{bhowmik2021}. Transactions
deemed valid by consensus can still encode malicious or fraudulent
activity, exposing a structural absence of intelligence-driven risk
assessment.

Cross-chain bridges introduce security and architectural challenges.
Their design---verification mechanisms, relayers, and smart
...
mobility of \emph{intelligence}: risk signals, behavioral history, and
...
Despite the rapid growth of decentralized finance and multi-chain
ecosystems, current blockchain infrastructures lack a unified mechanism
for sharing risk, trust, and behavioral context across chains. The
problem decomposes into five dimensions.

\subsubsection*{Signal fragmentation}
Wallet reputation, protocol exposure, and anomalous behavior are
visible only on the chain where they originate. Reputation systems
aggregate user interactions and feedback to derive trust
scores~\cite{HasanBB23}, but remain platform-specific: a user's
reputation on one chain does not transfer to another.

\subsubsection*{Reliability and manipulation}
Existing reputation frameworks suffer limitations in reliability,
portability, and manipulation resistance, including false feedback,
reputation fraud, Sybil attacks, and the absence of standardized
aggregation mechanisms~\cite{AlmasoudHH20}. Most systems do not treat
reputation as a transferable, composable digital asset, blocking
integration into broader decentralized ecosystems. Traditional
cryptography and access control address external threats but do not
capture the dynamic behavior of internal entities, motivating
trust-management systems that evaluate behavioral patterns and
interaction histories~\cite{ArshadKAKYZ23}.

\subsubsection*{Latency}
The absence of real-time, cross-chain risk intelligence delays
detection of malicious behavior~\cite{MishraP25,NahaZ24}. Blockchain
...
behaviors, or liquidity anomalies---are confined within individual
...
A wallet may exhibit a strong repayment or behavioral history on one
...
interoperable intelligence layer capable of aggregating, analyzing, and
...
\textbf{RQ2.} What classes of on-chain behavior are most predictive as
...
\textbf{RQ5.} To what extent can portable cross-chain behavioral
history support a practical notion of decentralized creditworthiness
...
\textbf{H2.} Multi-chain behavioral aggregation produces more
...
enable programmable coordination between chains, allowing decentralized
...
These protocols target \textit{transport}: they move assets and data
but do not account for the semantic meaning or contextual
interpretation of that data, and they lack mechanisms for propagating
risk signals, behavioral insights, or trust-related information. Value
moves seamlessly between ecosystems; the intelligence required to
interpret that movement remains confined to its origin chain. Security
compounds the limitation: cross-chain bridges expose attack surfaces
that have been repeatedly
exploited~\cite{ZhangZZL24,LiQXZZWX25,augusto2024sok}, and the
ecosystem still lacks mature public mechanisms for collecting,
organizing, and visualizing cross-chain operations at
scale~\cite{SantosAVC25}, constraining security analysis and timely
understanding of bridge activity.

\subsection{Cross-Chain Interoperability Approaches and Exploit-Risk
Propagation}\label{sec:rw:exploit-prop}

The interoperability protocols above decompose, for the purpose of
risk propagation, into four well-studied transport classes: lock-and-mint
bridges that escrow on the origin and mint a wrapped representation on
the destination; burn-and-mint bridges that destroy native supply on
the origin and reissue on the destination; light-client relays that
verify origin consensus proofs on the destination; and validator-set
messaging in which a fixed quorum signs cross-chain payloads, as in
LayerZero V1, Wormhole Guardian, and
Axelar~\cite{abs-2110-13871,zhang2023sok,augusto2024sok}.
Table~\ref{tab:exploit-propagation-comparison} contrasts each class
with the canary layer of Section~\ref{sec:arch} on six dimensions:
where in the message lifecycle a risk signal can fire
(\emph{detection point}); the typical block-time floor for that signal
to reach a remote consumer (\emph{propagation latency}); what the
recipient chain can do once a risk signal lands (\emph{mitigation
surface}); whether the signal is opaque, quorum-attested, or
economically bonded (\emph{verifiability}); whether an exploit is
contained to a single route, a single chain, or fans out across every
chain on which a wrapped or messaged artifact is in circulation
(\emph{post-exploit blast radius}); and a representative incident or
survey grounding the row (\emph{evidence}). The six dimensions are
chosen because each corresponds to an architectural decision the
canary layer makes differently: detection point shifts from
post-settlement to pre-settlement; propagation latency decouples from
asset-bridge finality and instead tracks the relayer hop $\tau_R$ of
Section~\ref{sec:uc:exploit}; the mitigation surface widens from
contract-pause primitives to typed, confidence-bearing consumer-side
actions; verifiability moves from black-box or quorum-attested toward
economic bonding under a Bittensor-style validator
loss~\cite{rao2021bittensor,opentensor2024yuma}; and the blast radius
is bounded by per-signal-type confidence bands and consumer-side
circuit breakers rather than by the topology of wrapped-asset issuance
or by a shared validator
set~\cite{augusto2024sok,zhang2023sok,wu2025safeguarding,werner2022sok,palaiokrassas2023multichain,daian2020flash}.

\begin{table*}[t]
\centering
\scriptsize
\caption{Cross-chain interoperability classes and the canary layer,
compared on six exploit-risk-propagation dimensions. Rows are the four
well-studied transport classes plus the canary layer of
Section~\ref{sec:arch}; columns are the dimensions defined in the
prose. Citations in the \emph{evidence} column point to representative
incident analyses or surveys, not to new measurements taken in this
paper.}
\label{tab:exploit-propagation-comparison}
\renewcommand{\arraystretch}{1.20}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.13\linewidth} >{\raggedright\arraybackslash}p{0.13\linewidth} >{\raggedright\arraybackslash}p{0.11\linewidth} >{\raggedright\arraybackslash}p{0.16\linewidth} >{\raggedright\arraybackslash}p{0.14\linewidth} >{\raggedright\arraybackslash}p{0.14\linewidth} >{\raggedright\arraybackslash}p{0.10\linewidth}@{}}
\toprule
\textbf{Approach class} &
\textbf{Detection point} &
\textbf{Propagation latency} &
\textbf{Mitigation surface} &
\textbf{Verifiability} &
\textbf{Post-exploit blast radius} &
\textbf{Evidence} \\
\midrule
Lock-and-mint bridge &
Post-settlement on destination; vault drain visible only on origin &
Origin finality + relayer delay (minutes) &
Pause or freeze the wrapped-asset contract; cannot un-mint already issued supply &
Black-box; relayer attestation, no risk semantics &
Fans out across every chain on which the wrapped asset is in circulation &
\cite{augusto2024sok,zhang2023sok} \\
\midrule
Burn-and-mint bridge &
Post-burn / post-mint at message verification on destination &
Origin finality + attester-quorum collection (minutes) &
Native asset reissue on destination; no automatic risk halt at the message level &
Quorum-attested by a fixed attester set; not bonded to risk evaluation &
Contained to the asset-flow channel; smaller fan-out than wrapped issuance &
\cite{augusto2024sok,wu2025safeguarding} \\
\midrule
Light-client relay &
At consensus-proof verification on destination; no risk-signal channel &
Source finality (epoch-level for PoS) &
Reject messages whose origin state proof fails; no risk-aware policy primitive &
Cryptographically attested at the consensus layer; no risk semantics &
Limited to the exploited route if a state proof is malformed; otherwise unbounded for app-layer compromise &
\cite{zhang2023sok,werner2022sok} \\
\midrule
Validator-set messaging (LayerZero V1, Wormhole, Axelar) &
At validator-quorum signature verification on destination &
Validator-set quorum collection (seconds to minutes) &
Pause individual application endpoints; no cross-application risk halt &
Quorum-attested by the validator set; not bonded to risk evaluation &
Route-to-route fan-out across every application sharing the validator set &
\cite{abs-2110-13871,zhang2023sok,wu2025safeguarding} \\
\midrule
RayaChain canary layer (this paper) &
At signal-emission time on origin, pre-settlement on destination &
Relayer hop $\tau_R$, decoupled from asset-bridge finality &
Typed, confidence-bearing consumer actions: collateral haircut, route refusal, circuit breaker before settlement &
Designed; layer-1 orchestration validated, layer-2 (validator-loss) deferred (Sec.~\ref{sec:vri}) &
Bounded by per-signal-type confidence band and consumer-side circuit breakers &
This paper; \cite{daian2020flash,palaiokrassas2023multichain} \\
\bottomrule
\end{tabular}
\end{table*}

The comparison admits one explicit limit. The \emph{verifiability}
column for the RayaChain canary layer reads ``designed; layer-1
orchestration validated, layer-2 deferred'', consistent with the
honest split of Section~\ref{sec:vri}: the orchestration path has
been measured under continuous unattended operation
(Table~\ref{tab:soak-telemetry}), but the three-component validator
loss has not yet been measured against a live multi-miner metagraph.
The row therefore states architectural intent rather than realized
economic bonding, and the load-bearing contrast with the four
traditional classes is the move from black-box or quorum-attested
verification toward an economically-scoreable
substrate~\cite{rao2021bittensor,opentensor2024yuma}---not a
present-tense claim of measured economic guarantees.

\subsection{Blockchain Analytics Platforms}

Blockchain analytics platforms have emerged as critical tools for
understanding on-chain behavior and detecting anomalies. Platforms
...
behavioral and security analysis~\cite{ChenLZCLLLZ20}. Empirical
studies confirm that cryptocurrency transaction networks exhibit
power-law degree distributions and non-assortative behavior, making
them suitable for large-scale behavioral and network-level
analysis~\cite{MotamedB19}. Recent work integrates security and
forensic perspectives with machine learning and behavioral clustering
...
operate primarily as \textbf{centralized intelligence layers}, relying
on proprietary data pipelines and off-chain processing. Second, their
analysis is largely \textbf{chain-specific}, interpreting behavioral
...
systems aggregate user feedback and behavioral data~\cite{HasanBB23};
blockchain-based implementations enhance transparency and
tamper-resistance, making them suitable for decentralized
...
behavior by analyzing transaction patterns, temporal features, and
...
\subsection{Decentralized Validation Networks}\label{sec:rw:dvn}

A separate strand of recent work asks how risk-relevant outputs
themselves can be validated in a decentralized, economically incentivised
...
architecture in which a Bittensor verification subnet decentralizes a
...
The relevance of decentralized validation networks to the canary layer
...
the validator-loss decomposition formalized in
...
provide deep insights into on-chain behavior; reputation systems offer
mechanisms for evaluating trust; and decentralized validation networks
...
\textit{intelligence}. Risk signals, behavioral patterns, and credit
...
intelligence layer that observes and analyzes on-chain behavior and
...
decentralized validation subnet makes the propagated signals
...
\begin{figure*}[t]
  ...
  \caption{End-to-end RayaChain canary signal flow. Anomalous on-chain
  events (\emph{Detect}, A1--A3) are normalized to typed
  \texttt{WalletSignal} envelopes, scored by the OmniRisk subnet under
  a Brier\,+\,Inconsistency\,+\,Calibration validator loss
  (\emph{Transform}, A4--A6), propagated cross-chain via signed OFT
  envelopes (\emph{Propagate}, A7--A9), and ingested by consumer
  systems that act under role-bound constitutional constraints
  (\emph{Decide\,+\,Respond}, A10--A13). \emph{Solid} nodes (Layer-1)
  are orchestration-validated by the $5{,}097$-round shadow soak of
  Section~\ref{sec:vri}; \emph{dashed blue} nodes (Layer-2)---
  validator-quorum signing, OFT routing, and quorum-verifying
  receivers---are designed but deferred per the Layer-1/Layer-2
  honesty split of Section~\ref{sec:vri}. The curved feedback arrow
  (A13\,$\to$\,A4) closes the economic self-correction loop: realized
  outcomes are the ground-truth signal for the validator loss.
  Decision diamonds gate the flow at score threshold (D1),
  envelope-signature verification (D2), and role-permission check
  (D3). The audit sub-rail anchors signed records from A2, A6, and
  A12 to a Merkle root. Cross-reference
  Table~\ref{tab:scoring-features} for the per-score feature
  decomposition at A5.}
...
interoperability primitives. The end-to-end flow is summarized in
Figure~\ref{fig:canary-signal-flow}: an originating chain emits a
signal; the OmniRisk subnet scores it; a relayer carries the signed
envelope and its provenance to a target chain; and a consuming
protocol or agent reads the signal under a typed schema before
deciding to act.
...
behavior logs, liquidity-pool state, bridge flow, and off-chain
...
The fields of \texttt{WalletSignal} are populated as follows, each
traceable back to a row of the feature-inputs table introduced later
(Table~\ref{tab:scoring-features}, Section~\ref{sec:vri:features}).
\texttt{origin} is the chain identifier of the emitting
signal-generation engine and is not scored; \texttt{wallet} is the
20-byte subject and is not scored. \texttt{type} is the
\texttt{WalletSignalType} discriminant and selects which row block of
Table~\ref{tab:scoring-features} the consumer dereferences---a
wallet-class signal points at the wallet \texttt{value\_bps} row,
while protocol- and market-class signals (under their equivalent IDL
structures) point at the protocol and market \texttt{value\_bps}
rows. \texttt{value\_bps} is the value sub-score whose feature inputs
are enumerated in the corresponding wallet, protocol, or market row;
\texttt{confidence\_q7} is the engine-level \texttt{confidence} score
whose feature inputs are enumerated in the
\texttt{confidence\_q7} (all) row. \texttt{window\_start} and
\texttt{window\_end} carry the validator scoring window $\Delta$ of
Section~\ref{sec:vri:formal} and are not themselves scored.
\texttt{prov\_root} is a Merkle commitment over the underlying feature
observations, so a verifying consumer can chase any \texttt{value\_bps}
or \texttt{confidence\_q7} back through Table~\ref{tab:scoring-features}
to a verifiable feature set without re-deriving the score. The
bridge-and-oracle anomaly indicator that resolves event $E_3$ in
Section~\ref{sec:vri:formal} is carried as a protocol-class signal
with a dedicated \texttt{type} discriminant and inherits the same
\texttt{prov\_root} provenance contract; its row in
Table~\ref{tab:scoring-features} states the cross-source redundancy
features that populate it.
...
Behavioral anomalies attached to a wallet identifier: sudden
...
graph-analytic
features~\cite{ChenLZCLLLZ20,SerenaFD22,MotamedB19}, behavioral
...
or pool identifier: utilization spikes, oracle divergence, bridge-flow
...
characterize how the same logical asset prices differently across
...
backing by composing the layer with a decentralized validation
subnet, and states the specification formally. Activities A4--A6 of
Figure~\ref{fig:canary-signal-flow}---miner forecast emission,
validator scoring against realized $E_1$--$E_4$ events, and signed
envelope production---are the locus of the verifiability argument
developed below.

\subsection{Verification by Decentralized Subnet}
...
We formalize the validator-side loss; the specification is reproduced
...
\subsection{Feature Inputs and Signal-Score Mapping}\label{sec:vri:features}

The validator loss of Section~\ref{sec:vri:formal} prices a forecast
against a resolver, but it does so over named scores whose feature
inputs the prose has so far left implicit.
Table~\ref{tab:scoring-features} enumerates the feature inputs that
feed each named score on the canary layer and labels which channel of
the validator loss the score contributes to. Four design choices
warrant explicit naming. First, each row binds to a concrete score
that lands in the wire envelope of Section~\ref{sec:arch}
(\texttt{value\_bps}, \texttt{confidence\_q7}) or to a resolver event
or validator-loss component, not to abstract risk concepts; this lets
a reviewer audit the methodology by reading the IDL and the table
together. Second, off-chain and grey-literature feeds are admitted
only in the market-level signal class, where narrative-shock fusion
divergence is empirically
informative~\cite{cong2022crypto}; wallet- and protocol-level scores
are restricted to on-chain inputs to keep the load-bearing scoring
path Sybil-resistant. Third, several feature classes are deliberately
excluded: centralized-exchange order-book microstructure (not
robust to the agent-as-adversary threat model of
Table~\ref{tab:threat-model}, since the venue itself is the
adversary), gas-price and mempool microstructure (trivially gameable
by an MEV-active miner~\cite{daian2020flash}), and price-only
technical indicators (already covered by the underlying
forecast pipeline of the Bittensor financial-prediction
precedent~\cite{taoshi2024ptn}, and not informative for the cross-chain
risk-and-credit propagation problem this layer is for).
Fourth, the validator-loss components are flagged \textsc{deferred}
consistently with the layer-2 honesty split of Section~\ref{sec:vri}:
the Brier, Inconsistency, and Calibration channels are specified
architecturally and not yet measured against a live multi-miner
metagraph. Downstream, the \texttt{value\_bps} score feeds collateral
haircuts (Section~\ref{sec:uc:credit}) and routing decisions for
autonomous agents; the bridge/oracle anomaly indicator gates the
exploit-propagation circuit breaker of Section~\ref{sec:uc:exploit};
and \texttt{confidence\_q7} gates the agent's constitutional action
surface, with each agent declaring a per-signal-type minimum
confidence below which a propagated signal is silently downweighted
rather than acted upon~\cite{bai2022constitutional}.

\begin{table*}[t]
\centering
\scriptsize
\caption{Feature inputs and validator-loss channel for each named
score on the canary layer. Each row corresponds to a score that lands
in the wire envelope (\texttt{value\_bps}, \texttt{confidence\_q7})
of Section~\ref{sec:arch} or to a component of the validator loss of
Section~\ref{sec:vri:formal}. \textsc{wired} denotes a score the
production signal-generation engine emits today; \textsc{deferred}
denotes an architectural specification not yet measured against a
live metagraph, consistent with the layer-1 / layer-2 split of
Section~\ref{sec:vri}.}
\label{tab:scoring-features}
\renewcommand{\arraystretch}{1.20}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.18\linewidth} >{\raggedright\arraybackslash}p{0.26\linewidth} >{\raggedright\arraybackslash}p{0.09\linewidth} >{\raggedright\arraybackslash}p{0.08\linewidth} >{\raggedright\arraybackslash}p{0.13\linewidth} >{\raggedright\arraybackslash}p{0.09\linewidth} >{\raggedright\arraybackslash}p{0.07\linewidth}@{}}
\toprule
\textbf{Score (field / channel)} &
\textbf{Feature inputs} & \textbf{Source} & \textbf{Range} &
\textbf{Prior work} & \textbf{VL channel} & \textbf{Status} \\
\midrule
\texttt{value\_bps} (wallet) &
exploit-contract interaction history; liquidity-withdrawal cascade
fingerprints; repayment-failure flags; transaction-graph centrality
drift &
on-chain & int16 bps, signed &
\cite{ChenLZCLLLZ20,MotamedB19,palaiokrassas2023multichain} &
Brier & \textsc{wired} \\
\texttt{value\_bps} (protocol) &
TVL contraction rate; oracle divergence; bridge-flow asymmetry;
utilization spikes &
on-chain, cross-chain aggregate & int16 bps, signed &
\cite{ZhangZZL24,augusto2024sok,gogol2024liquidity} &
Brier & \textsc{wired} \\
\texttt{value\_bps} (market) &
cross-chain withdrawal correlation; narrative-shock fusion divergence;
MEV / sandwich-active windows &
on-chain, grey-literature, social streams & int16 bps, signed &
\cite{cong2022crypto,daian2020flash} &
Brier, Inconsistency & \textsc{wired} \\
Bridge/oracle anomaly indicator (resolver $E_3$,
Sec.~\ref{sec:vri:formal}) &
cross-source redundancy disagreement on bridge-flow or oracle quote;
LayerZero substrate divergence flags; cross-chain wallet-reputation
mismatch on bridge counterparty &
on-chain, cross-chain & boolean per $\Delta$ &
\cite{augusto2024sok,ZhangZZL24,werner2022sok,palaiokrassas2023multichain}
&
Brier (resolver $E_3$) & \textsc{wired} \\
\texttt{confidence\_q7} (all) &
rolling realized accuracy of the signal class over $W$ samples;
signal-class stability across paired queries &
derived & Q0.7 in $[0,1]$ &
\cite{rao2021bittensor,hanson2003combinatorial} &
Calibration & \textsc{wired} \\
$\mathrm{Brier}(p_i,y)$ &
predicted probability $p_i$ and resolver event indicator
$y \in \{0,1\}$ over $\Delta$ &
derived (post-resolver) & $[0,1]$ &
\cite{hanson2003combinatorial,rao2021bittensor} &
Brier & \textsc{deferred} \\
$\mathrm{Inconsistency}(m_i)$ &
paired predictions on equivalent chain-state context &
derived & $[0,1]$ &
\cite{steeves2022incentivizing,opentensor2024yuma} &
Inconsistency & \textsc{deferred} \\
$\mathrm{Calibration}(c_i)$ &
per-bin gap between miner-stated $c_i$ and realized accuracy over
$W$ paired $(c_i,y)$ samples &
derived & $[0,1]$ &
\cite{hanson2003combinatorial} &
Calibration & \textsc{deferred} \\
\bottomrule
\end{tabular}
\end{table*}
...
each as a row over capability, architectural defense, and the residual
...
the adversary's assumed capability, the architectural defense, and the
...
preempts the reading of the paper as a uniformly empirical artifact:
...
2026-05-04~\cite{omnirisk2026soak}. Over a 57-hour window the run recorded \textbf{$5{,}097$ successful
rounds} and \textbf{zero authentication failures} across eleven
autonomous Auth0 token refreshes (Table~\ref{tab:soak-telemetry}). The
shadow run validates the orchestration path: the canary layer emits
signed signals to a Bittensor-style validation substrate continuously,
under autonomous credential rotation, without operator intervention.
...
artifact~\cite{omnirisk2026soak}. The table is the smallest possible
...
\paragraph*{Canary-layer behavior.}
...
\paragraph*{Scenario.}
A wallet has a strong repayment and behavioral history on chain~$A$
...
and~\ref{sec:vri} is deployable; the empirical evaluation program is
...
Second, wallet behavior logs and labelled fraud cohorts from the
...
value and arbitrary messages, the canary layer transports evaluated
risk and credit context under a typed schema with confidence and a
window of validity. We formalized the canary-layer abstraction,
defined a three-tier signal taxonomy across wallet-, protocol-, and
market-level classes, and grounded the architecture in three
use-case scenarios---exploit propagation, credit portability, and
liquidity stress---together with a falsifiable metric set covering
detection latency, signal accuracy, and cross-chain coverage. We
then demonstrated that the layer becomes verifiable in expectation
when paired with a Bittensor-style decentralized validation subnet,
and anchored the verification claim in an orchestration shadow soak
($5{,}097$ successful rounds, zero authentication failures, eleven
autonomous credential rotations); the layer-2 economic verification
of the validator loss is named honestly as the smallest deferred
experiment. The contribution is architectural: a canary layer that
lets autonomous agents on heterogeneous chains act on the same
evaluated context, rather than on chain-local fragments of it.
...
\textbf{Ahod Alghuried:} contribution roles to be confirmed at the
time of submission. ORCIDs: Basel Magableh,
\texttt{0000-0003-2337-637X}; Ahod Alghuried, ORCID TBC.
...
The Bittensor shadow-soak telemetry summarized in
Table~\ref{tab:soak-telemetry} is committed alongside this manuscript
in the engineering source-of-truth
artifact~\cite{omnirisk2026soak}. The signal-schema definition of
...
acquisition under restricted license. All on-chain data referenced are
...
%% (A.~Alghuried) email, ORCID, and IEEE membership grade are confirmed.
%% Surname and affiliation were confirmed by CEO directive 2026-05-07.
